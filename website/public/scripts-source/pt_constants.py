...
Perturbative corrections:
  ...
  - Echo VP (R28)    : delta(1/alpha) = -gamma_3*alpha*sum(sin2_p*gamma_p, p echo)
                         p echo = {11,13} (inactive in the sieve, gamma<s)
                         = vacuum polarization by echo "particles"
  - Echo mass (R29b): delta(ratio)/ratio = -delta_SE*alpha*C_geom*beta_echo
  ...
  - Tau cross-branch (R34b): ratio_tau_mu *= (1 + alpha_s * beta_echo * eps)
                         Tau crosses vertex/edge (hadronic modes)
                         alpha_s = edge coupling, beta_echo = echo weight
                         Cross-branch echo VP: 0.27% off, 2.43 sigma -> 0.00 sigma
...
# Echo screening [DERIVED, p=2 architecture]
...
# R29b: Echo VP for lepton masses [DERIVED, see monograph]
...
#   delta(ratio)/ratio = -delta_SE * alpha * C_geom * beta_echo
# = 3 (spatial dimensions)
_N_spatial = len(PRIMES_ACTIFS)
_cos2_thetaW = 1.0 - sin2_thetaW
_C_geom_mass = mu_star + 2**_N_spatial + _cos2_thetaW
_echo_VP_mass = delta_SE * alpha_EM * _C_geom_mass * _beta_echo
ratio_mu_e = ratio_mu_e * (1 - _echo_VP_mass)

m_mu = m_e * ratio_mu_e

# R34b: Tau-specific radiative correction (cross-branch echo VP) [DERIVED]
...
#   beta_echo = echo VP weight (sin^2_p * gamma_p, p in {11,13})
#   eps = universal running (beta_0 * alpha / (4*pi))
#   delta_tau = alpha_s * beta_echo * eps = cross-branch echo VP
#   Tau "sees" the echoes via the strong coupling (mod 3, r=0 allowed)
_alpha_s_tau = sin2_therm[3] / (1.0 - alpha_EM)
_delta_tau_cross = _alpha_s_tau * _beta_echo * eps
...
# R54: Echo NNLO -- echo screening of c->b vertex [DER-PHYS]
#   gamma_11 . alpha_EM: first echo prime (p=11) screens c->b vertex
#   Negative dual of echo V_td reinforcement (same magnitude, opposite sign)
V_cb = V_cb * (1 - _gamma_echo[11] * alpha_EM)
...
# R54: Echo NNLO -- echo reinforcement of b->d mixing [DER-PHYS]
#   gamma_11 . alpha_EM: first echo prime (p=11) reinforces b->d mixing
#   b = 5th flavor, echo zone. Positive dual of echo VP (screening -> reinforcement)
#   Non-repetition: gamma_11 alone (first echo), not beta_echo
V_td = V_td * (1 + _gamma_echo[11] * alpha_EM)
...
# Dimensional relation (not causal derivation of G). R39 = echo correction.
#   delta_holo = delta_SE * (sin2_3/sin2_7) * beta_echo
#   sin2_3/sin2_7 = color/space projection ratio (p=3 vs p=7)
#   Same mechanism as R28 (VP) and R29b (mass): 4th echo order
_factor_G_holo = sin2_stat[3] / sin2_stat[7]
_delta_holo_G = delta_SE * _factor_G_holo * _beta_echo

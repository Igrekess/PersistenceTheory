...
summary: 'L’espace n’a pas trois dimensions par hasard. La PT donne une raison arithmétique : il y a exactement trois nombres premiers actifs à l’attracteur réduit — $\{3, 5, 7\}$ — et chacun ouvre une direction.'
level: L1
reading_minutes: 6
...
Quand on calcule $\gamma_p$ à l’attracteur réduit $\mu^* = 15$, on trouve :
...
L’attracteur réduit $\mu^* = 15$ n’est pas un choix : c’est le **seul** sous-ensemble de premiers qui satisfait la condition d’auto-cohérence dans le secteur informationnel
...
> 🧮 **[→ Calculatrice γ_p(μ)](/calculator)** — déplacer le curseur μ et voir
> comment la liste des premiers actifs change. Au point μ* = 15, exactement
> $\{3, 5, 7\}$ passent le seuil $\gamma_p > 1/2$.

## Voir aussi

- [Essai — D’où vient s = 1/2 ?](/essays/where-s-comes-from)
- [Essai — Qu’est-ce qu’un premier d’écho ?](/essays/echo-primes) — le sort de $\{11, 13\}$
- [Théorème T5 — Point fixe μ* = 15](/theorems/T5)

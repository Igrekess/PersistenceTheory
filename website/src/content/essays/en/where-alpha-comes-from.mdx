...
$\alpha_{\mathrm{EM}}$ lives on the $q_+$ branch (couplings). If we computed the same product on the $q_- = e^{-1/\mu^*}$ branch (geometry), we would not get $\alpha_{\mathrm{EM}}$ — we would get geometric observables (Newton’s constant, quark masses, CKM mixing). This bifurcation is what separates the lepton/vertex sector from the quark/propagator sector.

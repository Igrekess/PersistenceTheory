...
summary: 'Space does not have three dimensions by accident. PT gives an arithmetic reason: there are exactly three active primes at the attractive fixed point — $\{3, 5, 7\}$ — and each opens one direction.'
level: L1
reading_minutes: 6
...
Computing $\gamma_p$ at the attractive fixed point $\mu^* = 15$:
...
The attractive fixed point $\mu^* = 15$ is not a choice: it is the **only** subset of primes that satisfies the self-consistency condition

...
This essay exists in two versions on this same page. The <strong>plain</strong> version (no formulas beyond the elementary) appears at the <em>Plain</em> reading depth. The <strong>standard / technical</strong> version (with the uniqueness proof, D17b references, epistemic status) appears at the <em>Standard</em> and <em>Technical</em> depths. Use the depth selector at the bottom-right ↘ to switch.
</aside>
...
The simultaneous appearance of $3^2$ and $2^3$ in this structure is *not* a parameter choice. It is the unique pair of integers where the gap between powers equals $1$ — exactly Mihailescu — and that is what selects $N_{\text{gen}} = 3$ in the PT cascade, and not $2$ or $4$.
